package com.example.topmovies.presentation.adapter
...
class MoviesAdapter(
    ...
    override fun onBindViewHolder(
        holder: MovieViewHolder, position: Int, payloads: MutableList<Any>
    ) {
        if (payloads.isEmpty()) {
    ...
    class MovieViewHolder(
        ...
        fun bindChances(changes: PayloadChange) = when (changes) {
        ...
        private fun getChanges(id: String, favorite: Boolean) {
            with(binding.imageButtonItemFavoriteIcon) {
                icon = AppCompatResources.getDrawable(
                    binding.root.context, getFavoriteImageResource(favorite)
                )
                setOnClickListener {
        ...
        fun bind(movie: Movie) = with(binding) {
            ...
            GlideApp.with(circleAvatarViewItemLayoutMovieImage).asBitmap()
