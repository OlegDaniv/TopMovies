...
class MovieRepository constructor(
    ...
    fun getMovies(callback: (List<MovieEntity>) -> Unit) {
        executor.execute {
            callback(moviesDao.getMovies())
        }
    }

    fun getFavoriteMovie(callback: (List<MovieEntity>) -> Unit) {
        executor.execute {
            callback(moviesDao.getFavoriteMovies(true))
        }
    }
    ...
    fun upsertMovies(movies: List<Movie>) {
        movies.forEach { movie ->
            getMovieById(movie.id, callback = {
                it?.let { updateMovie(it.id, it.rank, it.rankUpDown) }
    ...
    private fun updateMovie(id: String, rank: String, rankUpDown: String) {
        executor.execute { moviesDao.updateMovie(id, rank, rankUpDown) }
    }

    private fun getMovieById(id: String, callback: (MovieEntity?) -> Unit) {
        executor.execute { callback(moviesDao.getMovieById(id)) }
    }

    private fun insert(movie: MovieEntity) {
